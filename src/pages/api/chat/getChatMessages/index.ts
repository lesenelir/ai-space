...
const handleGetChatMessages = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(200).json({ status: 'Get Chat Messages successfully!', chatMessages })
  } catch (e) {
    return res.status(500).json({ error: e })}
}

router.post(handleGetChatMessages)
